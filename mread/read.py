...
def jrdp3d(fieldlinefilename):
    global t,nx,ny,nz,_dx1,_dx2,_dx3,rho,B1,B2,B3
    #read image
...
def grid3d(dumpname): #read gdump: header and body
    ...
    gin.close()
    #read gdump
    gd = np.loadtxt( "dumps/gdump", dtype=float, skiprows=1, unpack = True ).view().reshape((-1,nx,ny,nz), order='F')
    ...
    conn=gd[9:73].view().reshape((4,4,4,nx,ny,nz), order='F')
    conn.transpose(2,1,0,3,4,5)  #get the right order of indices
