...
def plot_stampede(fntsize=20,dosavefig=1):
    #savio
    n_list = [      20,    100,               2000,           4000]
    s_list = [  454730, 2047743,   0.9534*41832930,   0.9213*2*41832930]
    n_list = np.array(n_list,dtype=np.float64)
    s_list = np.array(s_list,dtype=np.float64)
    plt.clf()
    n_array = 10**np.linspace(1,5,100)
    plt.plot(n_list, 1.0*s_list[0]/n_list[0]*n_list,
             "k:",lw=2,label=r"$\rm ideal\ scaling$")
    plt.plot(n_list, s_list,"-s",lw=2,color="green",label=r"$\rm Stampede\ scaling$",markersize=10)
    plt.xscale("log")
    plt.yscale("log")
    plt.xlim(10,1e4)
    plt.ylim(1e5,2e8)
    ...
    for label in ax.get_xticklabels() + ax.get_yticklabels():
        label.set_fontsize(fntsize)
    plt.savefig("stampede_scaling.pdf",
                bbox_inches='tight',pad_inches=0.06,dpi=300)
...
def createnewdic(**kwargs):
    ...
    cdata3 = np.array([np.array(cm.datad[whichmap3]['red']),
              np.array(cm.datad[whichmap3]['green']),
              np.array(cm.datad[whichmap3]['blue'])])
    ...
    if redver==3:
        ...
        cdata[:,:2,0] = np.linspace(0.,0.25,2)[None,:]
        cdata[:,2:5,0] = np.array([0.55, 0.6, 0.65])[None,:]
        cdata[:,5:9,0] = np.linspace(0.7,0.78,4)[None,:]
        ...
        cdata[:,2:5,0] = np.array([0.55, 0.6, 0.65])[None,:]
        cdata[:,5:9,0] = np.linspace(0.7,0.78,4)[None,:]
        cdata[:,9:10,0] = np.array([0.8])[None,:]
...
def lasota_plots(doreload=0,dofig=0,dokomistag=0,avoidbc=0,fntsize=20):
    if 'gv3' not in globals() or 'rho' not in globals() or doreload:
        grid3d("gdump")
        rdo("dumplast")
    #Omega_F
...
def stag_compute_analytic(**kwargs):
    Delta = r**2+a**2-2*r
...
def eminusomegal_plot(fntsize=20,useavgs=0,doreload=0,fname=None,dofig=False,dokomistag=False,avoidbc=0):
    ...
    mksimplevecstream(vec, len=mylen,density=1,downsample=1,cb=False,vmin=-6,vmax=0.5,dobhfield=10,dodiskfield=False,minlenbhfield=0.2,minlendiskfield=0.5,dsval=0.005,color='r',linewidth=1,startatmidplane=False,arrowsize=arrowsize,populatestreamlines=populatestreamlines,domirrory=domirrory,donanout=True)
    ...
    if avoidbc:
        ...
        populatestreamlines=1
    if 0: # switch off the energy streamlines
...
def plotEr(dofig=0,fntsize=20,useavgs=0):
    ...
    plt.clf()
    varx = rloc[:,0,0]
    vary = prefac*(-tud[1,0]*gdet).sum(1).sum(-1)*_dx2*_dx3*nz
