...
def mkframe(fname,ax=None,cb=True,vmin=None,vmax=None,len=20,ncell=800,pt=True,shrink=1,dostreamlines=True):
    ...
    if ax == None:
        ...
        if not dostreamlines:
            ...
            # if t < 1500:
            #     lw *= ftr(ilrho,-2.,-1.9)
            fstreamplot(yi,xi,iBR,iBz,density=1,linewidth=lw,ax=ax,detectLoops=True,dodiskfield=False,dobhfield=True,startatmidplane=True,a=a)
...
def mkframexy(fname,ax=None,cb=True,vmin=None,vmax=None,len=20,ncell=800,pt=True,shrink=1,dostreamlines=True):
    ...
    if ax == None:
        ...
        if dostreamlines:
            ...
            #     lw *= ftr(ilrho,-2.,-1.9)
            lw *= ftr(iaphi,0.001,0.002)
            fstreamplot(yi,xi,iBx,iBy,density=1,linewidth=lw,detectLoops=True,dodiskfield=False,dobhfield=False,startatmidplane=False,a=a)
        ax.set_xlim(extent[0],extent[1])
        ax.set_ylim(extent[2],extent[3])
    #CS.cmap=cm.jet
    #CS.set_axis_bgcolor("#bdb76b")
